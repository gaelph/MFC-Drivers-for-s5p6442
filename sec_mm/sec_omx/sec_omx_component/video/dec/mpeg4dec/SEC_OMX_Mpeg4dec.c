...
static int Check_Mpeg4_Frame(unsigned char *pInputStream, int buffSize, OMX_U32 flag, OMX_BOOL bPreviousFrameEOF, OMX_BOOL *pbEndOfFrame)
{
    unsigned int len, readStream;
    unsigned startCode;
...
OMX_ERRORTYPE SEC_MFC_Mpeg4Dec_Init(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    pStreamBuffer = SsbSipMfcDecGetInBuf(hMFCHandle, MPEG4_DEC, &pStreamPhyBuffer, MFC_DATA_BUF_SIZE);
...
OMX_ERRORTYPE SEC_MFC_Mpeg4_Decode(OMX_COMPONENTTYPE *pOMXComponent, SEC_OMX_DATA *pInputData, SEC_OMX_DATA *pOutputData)
{
    ...
    if (pMpeg4Dec->hMFCMpeg4Handle.bConfiguredMFC == OMX_FALSE) {
        ...
        configValue = 1;
        SsbSipMfcDecSetConfig(hMFCHandle, MFC_SET_CONFIG_MP4_DBK_ON, &configValue);
    ...
    if (returnCodec == MFC_RET_OK) {
        ...
        bufWidth =  outputInfo.img_width;
        bufHeight =  outputInfo.img_height;
