...
//Add by Le Bidou : we need at least the pps frame
    ...
    while (1) {
        ...
        if (preFourByte == 0x00000001 || (preFourByte << 8) == 0x00000100) {
            ...
            if (naluStart == 0) {
                ...
                if (naluType == NAL_SLICE || naluType == NAL_IDR_SLICE)
#else
                if (naluType == NAL_SLICE || naluType == NAL_IDR_SLICE || naluType == NAL_SPS || naluType == NAL_PPS)
#endif
                    naluStart = 1;
            } else {
#ifdef OLD_DETECT
    ...
    OMX_OUT OMX_INDEXTYPE *pIndexType)
{
    ...
    if (SEC_OSAL_Strcmp(cParameterName, "OMX.SEC.index.ThumbnailMode") == 0) {
        ...
        *pIndexType = OMX_IndexVendorThumbnailMode;
        ...
        ret = pOMXComponent->GetExtensionIndex(hComponent, cParameterName, pIndexType);
    }
...
OMX_ERRORTYPE SEC_MFC_H264Dec_Init(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    pStreamBuffer = SsbSipMfcDecGetInBuf(hMFCHandle, H264_DEC, &pStreamPhyBuffer, MFC_DATA_BUF_SIZE);
    ...
    pSECComponent->processData[INPUT_PORT_INDEX].allocSize = MFC_DATA_BUF_SIZE;
...
OMX_ERRORTYPE SEC_MFC_H264_Decode(OMX_COMPONENTTYPE *pOMXComponent, SEC_OMX_DATA *pInputData, SEC_OMX_DATA *pOutputData)
{
    ...
    if (pH264Dec->hMFCH264Handle.bConfiguredMFC == OMX_FALSE) {
        ...
        if (returnCodec == MFC_RET_OK) {
            ...
            unsigned int actualWidth = imgResol.width - cropInfo.crop_left_offset - cropInfo.crop_right_offset;
            unsigned int actualHeight = imgResol.height - cropInfo.crop_top_offset - cropInfo.crop_bottom_offset;
            ...
            SEC_OSAL_Log(SEC_LOG_TRACE,"MFC Couldn't init");
            goto EXIT;
        }
    }
    ...
    if (returnCodec == MFC_RET_OK) {
        ...
        bufWidth = outputInfo.img_width - outputInfo.crop_left_offset - outputInfo.crop_right_offset;
        bufHeight = outputInfo.img_height - outputInfo.crop_top_offset - outputInfo.crop_bottom_offset;
        ...
        /* Added by lebidou 
           This is to prevent the scrambled screen on start playing and corrects playback after pause + screen went off sequence
         */
        if (pInputData->timeStamp == (long long)0)
            status = MFC_GETOUTBUF_DECODING_ONLY;
    ...
    if (pOutputData->dataLen > 0)
    {
...
#endif
        {
            ...
            if ((outputInfo.crop_top_offset == 0) && (outputInfo.crop_bottom_offset == 0) && (outputInfo.crop_left_offset == 0) && (outputInfo.crop_right_offset == 0)) {
                ...
                void * CbPlane = CrPlane + (((bufCopySize + 3) & (~3)) / 4);
                int count = 0;
                ...
                for (count = (outputInfo.crop_top_offset / 4); count < ((bufHeight + outputInfo.crop_top_offset)); count ++) {
                    int srcOffset = (((outputInfo.buf_width + 3) & (~3)) / 4) * count;
                    int dstOffset = frameSize + (((bufWidth + 3) & (~3)) / 4) * count;
                    SEC_OSAL_Memcpy(pOutBuf + dstOffset, (void *)CrPlane + srcOffset, (((bufWidth + 3) & (~3)) / 4));
                }
                    
                // Copy the Cb channel
                for (count = (outputInfo.crop_top_offset / 4); count < ((bufHeight + outputInfo.crop_top_offset)); count ++) {
                    int srcOffset = (((outputInfo.buf_width + 3) & (~3)) / 4) * count;
                    int dstOffset = frameSize + (((frameSize + 3) & (~3)) / 4) + (((bufWidth + 3) & (~3)) / 4) * count;
                    SEC_OSAL_Memcpy(pOutBuf + dstOffset, (void *)CbPlane + srcOffset, (((bufWidth + 3) & (~3)) / 4));
                }
            }
...
OSCL_EXPORT_REF OMX_ERRORTYPE SEC_OMX_ComponentInit(OMX_HANDLETYPE hComponent, OMX_STRING componentName)
{
    ...
    for(i = 0; i < ALL_PORT_NUM; i++) {
        ...
        pH264Dec->AVCComponent[i].eLevel     = OMX_VIDEO_AVCLevel3;
    }
