...
static unsigned int g_lcd_width = 240;
static unsigned int g_lcd_height = 400;
static unsigned int g_lcd_bpp = 16;
...
int v4l2_overlay_open(const char *device_name)
{
    LOG_FUNCTION_NAME
        ...
        uint32_t w, uint32_t h)
{
    ...
    case HAL_PIXEL_FORMAT_CUSTOM_YCbCr_420_SP:
        pix->pixelformat = V4L2_PIX_FMT_YUV420;//V4L2_PIX_FMT_NV12T;
        break;
    case HAL_PIXEL_FORMAT_CUSTOM_YCrCb_420_SP:
        pix->pixelformat = V4L2_PIX_FMT_YUV420;//V4L2_PIX_FMT_NV21;
        break;
    default:
        return -1;
    }
                      ...
                      uint32_t addr)
{
    ...
    ret = v4l2_overlay_s_fbuf(fd, 90, addr);
    if (ret)
        return ret;
...
int v4l2_overlay_s_fbuf(int fd, int rotation, uint32_t addr)
{
    struct v4l2_framebuffer fbuf;
    int ret;
